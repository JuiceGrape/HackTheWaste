	...
	void Update ()
	{
		if(usingController)
		{
			if(Vector3.Magnitude(new Vector3(Input.GetAxis("RightStickHorizontal"),Input.GetAxis("RightStickVertical"), 0)) > 0.6f)
            	transform.localEulerAngles = -Mathf.Sign(Input.GetAxis("RightStickVertical")) * Vector3.back * Vector3.Angle(Vector3.right, Vector3.Normalize(new Vector3(Input.GetAxis("RightStickHorizontal"), Input.GetAxis("RightStickVertical"), 0)));
		}
		else
		{
